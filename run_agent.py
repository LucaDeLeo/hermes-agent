...
class AIAgent:
    ...
    def __init__(
        ...
        # If config selects the Claude Code harness and provider is native Anthropic,
        # connect the SDK session up front so the first turn can resume properly.
        try:
            from hermes_cli.config import load_config as _lc_init
            if (
                self._get_harness_mode(_lc_init()) == "claude_code"
                and self._is_native_anthropic_for_harness
            ):
                self._harness_sdk_session_id = self._load_sdk_session_id()
                self._create_harness()
        except Exception as exc:
            logger.debug("Harness init at construction time failed: %s", exc)
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        _result = _switch_model(self, new_model, new_provider, api_key, base_url, api_mode)
        # Fork-only: re-evaluate harness after provider swap. Tear down stale
        # SDK session (it's bound to the previous provider's auth) and rebuild
        # for the new provider when harness mode is still active and the new
        ...
        if self._is_native_anthropic_for_harness:
            try:
    ...
    def run_conversation(
        ...
        """Forwarder with harness dispatch — see ``agent.conversation_loop.run_conversation``."""
        # Fork-only: route to Claude Code harness when active.
        if getattr(self, "_claude_agent_session", None) is not None:
            # Mirror the minimum preprocessing the harness path needs.
            try:
                self._ensure_db_session()
            except Exception:
                pass
            ...
            # Memory prefetch + plugin context injection (mirrors HEAD's dispatch site).
            _injections: list = []
            if self._memory_manager:
                try:
                    _turn_msg = (
